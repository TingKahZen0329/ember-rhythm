...
def create(parent):
    ...
    def select_audio_file(entry_widget):
        ...
        word_text = word_entry.get()
        if not word_text:
            messagebox.showwarning("警告", "請先輸入單字，才能添加聲音檔案。", parent=add_window)
            return
    ...
    def clear_audio_file(entry_widget):
        entry_widget.config(state='normal')
        entry_widget.delete(0, tk.END)
        entry_widget.config(state='readonly')
    ...
    ttk.Button(form_frame, text="瀏覽...", command=lambda: select_audio_file(word_audio_entry), bootstyle="info-outline").grid(row=4, column=2, sticky="w")
    ttk.Button(form_frame, text="清除", command=lambda: clear_audio_file(word_audio_entry), bootstyle="danger-outline").grid(row=4, column=3, sticky="w", padx=5)
    ...
    ttk.Button(form_frame, text="瀏覽...", command=lambda: select_audio_file(example_audio_entry), bootstyle="info-outline").grid(row=5, column=2, sticky="w")
    ttk.Button(form_frame, text="清除", command=lambda: clear_audio_file(example_audio_entry), bootstyle="danger-outline").grid(row=5, column=3, sticky="w", padx=5)
